...
- [Redpanda Load Factor](grafana-dashboards/Redpanda-Load-Factor-Dashboard.json) - Shows utilization relative to
capacity (load factor) for key resources: CPU (reactor), IO scheduler, disk IOPS, memory, network bandwidth, and
client connections, to help identify which resource a cluster will exhaust first.
...
- [Redpanda Stretch Cluster Dashboard](grafana-dashboards/Redpanda-Stretch-Cluster-Dashboard.json) - Observability for
Redpanda stretch clusters managed by the [Redpanda Operator](https://github.com/redpanda-data/redpanda-operator):
cross-cluster raft health, StretchCluster member status, and operator reconcile health. Generated by
`rpk generate grafana-dashboard --dashboard operations-stretch`; the operator metrics it visualizes are documented in the
operator repository's [operator-metrics.md](https://github.com/redpanda-data/redpanda-operator/blob/main/docs/operator-metrics.md).
